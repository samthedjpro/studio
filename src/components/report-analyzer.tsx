...
import { UploadCloud, Loader2, FileText, AlertTriangle, Hospital, Phone, MapPin } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card'
import { useToast } from '@/hooks/use-toast'
import { analyzeReportAction, findHospitalsAction } from '@/app/actions'
import type { AnalyzeReportOutput } from '@/ai/flows/analyze-report'
...
export default function ReportAnalyzer() {
  ...
  const handleAnalyze = async () => {
    ...
    setHospitals(null)
    try {
      const res = await analyzeReportAction({ photoDataUri: imageDataUri })
      setResult(res)
    } catch (error) {
      toast({
  ...
  return (
    <Card className="w-full shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Upload an image of a medical report (e.g., X-ray, CT scan, blood report) for an AI-powered analysis.</CardDescription>
      </CardHeader>
      <CardContent className="p-6">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground">
              {isDragging ? 'Drop the image here' : 'Drag & drop a report image, or click to select'}
            </p>
          </div>
        </div>
        ...
        {isLoading && !result && (
            <div className="mt-6 text-center">
                ...
                <p className="mt-2 text-muted-foreground">The AI is analyzing the report...</p>
            </div>
        )}
          ...
          <div className="mt-8 space-y-6">
            <Card>
              ...
              <CardContent className="pt-4">
                <p className="text-base text-foreground/90">{result.analysis}</p>
              </CardContent>
            </Card>
